...
We can see that each of the histograms are right-skewed, however when the *binwidth = 15* parameter is set, we can see that it is not completely right skewed. A majority of the departure delays occur in the second bar plotted.
...
```{r dest_sfo-month_feb}
sfo_feb_flights <- nycflights |>
  filter(dest == 'SFO', month == 2)

knitr::kable(sfo_feb_flights %>% 
  summarise(total_flights = n()))
```

There are total of 68 flights headed to SFO in February.
...
```{r dest_sfo-month_feb-histogram}
length_x <- nrow(sfo_feb_flights)
iqr_x <- IQR(sfo_feb_flights$arr_delay, na.rm = TRUE)

bin_size <- 2 * iqr_x / length_x^(1/3)

ggplot(sfo_feb_flights, aes(x = arr_delay)) + 
  geom_histogram(bins = round(bin_size, 0))
```

```{r dest_sfo-month_feb-stats}
knitr::kable(sfo_feb_flights %>% 
  summarise(., arr_delay_median = median(arr_delay), arr_delay_iqr = IQR(arr_delay)))
```

The distribution of the arrival delays is right-skewed. When a distribution is not normally distributed we should use the median and IQR instead of a traditional mean and standard deviation as potential outliers are weighing heavily on it. The mdeian is -11 and the IQR is 23.25.
...
```{r summary-arr_delays}
knitr::kable(sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_dd = median(arr_delay), iqr_dd = IQR(arr_delay)))
```

Both Delta (DL) and United Airlines (UA) have the most variable arrival delays as their IQR is 22.00.
...
If you choose the month with the lowest mean departure delay, you are taking into account flight departure delays that may be abnormal and may increase a specific month more than usual, in turn, possibly ignoring another month that would favor you better. However, choosing the lowest median departure delay may not account for these unusual occurrences that could further delay your flights in general. For example, if the flight year data had an unusually warm winter months with no snowfall, then these months may look like a good month to avoid delays, ignoring the fact snowfall is a lurking variable to account for.  
...
Based on on-time departure rates, LaGuardia Airport (LGA) would be best at 72.8%
...
``` {r scatterplot-avg_speed-distance}
nycflights %>% 
  ggplot(aes(x = avg_speed, y = distance)) +
  geom_point()
```  

We can see that as the average speed increases there is an exponential increase in the distance traveled. 
...
Using a histogram we can see that departure delays are right-skewed when accouting for arrivals to be either on-time or earlier. 

```{r histogram-plot, echo=FALSE}
length_x <- nrow(sfo_feb_flights)
iqr_x <- IQR(sfo_feb_flights$arr_delay, na.rm = TRUE)

bin_w <- 2 * iqr_x / length_x^(1/3)

filtered_carriers %>% 
  filter(arr_delay <= 0) %>% 
  ggplot(aes(x = dep_delay)) +
  geom_histogram(bins = bin_w)
```

We can see when flying with American, Delta or United airlines, to arrive on-time or earlier, your flight should roughly depart 2-4 minutes earlier. 

```{r summary-stats-filtered, echo=FALSE}
knitr::kable(filtered_carriers %>% 
  group_by(carrier) %>% 
  filter(arr_delay <= 0) %>% 
  summarise(dep_median = median(dep_delay), dep_iqr = IQR(dep_delay)))
```